...
parser.add_argument("--inputFile", type=str, help="input harvest file", default = "Outputs/36.5ifb/GG_direct/GG_direct_combined_fixSigXSecNominal__1_harvest_list")
...
def main():
	...
	for whichEntry in zi:
		...
		for contour in contourList:
			...
			if graph.Integral()<10000.:
				continue
			cleanedContourList.append(contour)

		for i,contour in enumerate(cleanedContourList):
			if (contour[1]>contour[0]).all():
				continue
			graph =	ROOT.TGraph(len(contour[0]), contour[0].flatten('C'), contour[1].flatten('C') )
...
def harvestToDict( harvestInputFileName = "" ):
	...
	descriptionFileName = harvestInputFileName.rsplit('/', 1)[0] + "/summary_harvest_tree_description.*"
...
def interpolateMassPlane(massPlaneDict = {}, interpolationFunction = "linear"):
	...
	zValues = {}
	for whichEntry in ["CLs","CLsexp","clsu1s","clsd1s"]:
