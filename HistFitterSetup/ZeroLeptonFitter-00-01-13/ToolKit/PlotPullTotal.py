...
#    allAna=["SR2jl","SR2jm","SR2jt","SR4jt","SR5j","SR6jm","SR6jt"]
    ...
    hPulls = TH1F("Pulls total","Pulls total",30,-3,3)
    hPulls.GetYaxis().SetTitle("Number of regions");
    hPulls.GetXaxis().SetTitle("(n_{obs} - n_{pred}) / #sigma_{tot}");
...
#        print "chi2 ",channel,chi2/len(allVRsForChi2),chi2bis/len(allVRsForChi2)
    ...
    canvas.Print("SummaryPull2.eps")
    canvas.Print("SummaryPull2.gif")
    canvas.Print("SummaryPull2.pdf")


    canvas = TCanvas("canvas","canvas",1400,800)
    text3 = TLatex()
    text3.SetTextFont(42);
    text3.SetTextSize(0.05);
    text3.SetTextColor(kBlack);
    text3.SetNDC(True);
    nRegions = hPulls.Integral(1,30)
    print("nRegions " + str(nRegions))
    hPulls.Draw()
#    text3.DrawLatex(0.22,0.83,"Total Regions: " + str(int(nRegions)));
    canvas.Print("SummaryPullTotal.pdf")
